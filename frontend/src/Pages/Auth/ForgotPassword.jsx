...
import SmilingFaceLoader from "../../Components/SmilingFaceLoader"; // Ensure this path is correct

const ProtectedRoute = () => {
  const { user, loading } = useAuth();
  const navigate = useNavigate();

  // Loading State - matching the loader used in ForgotPassword
  if (loading) {
    return <SmilingFaceLoader />;
  }

  // If not logged in, show "Login Required" UI matching the ForgotPassword theme
  if (!user) {
    return (
